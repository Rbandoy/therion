...
const TransactionService = {
		...
		try {
			await Notification.sendEmail({
				message: "<p>Thank you for sending remittance at Unified Products and Services </p>" +
					`<p>Please click on the link below to view your receipt: http://10.10.1.98:8090/portal/ecash_to_paymaya_receipt/${trackingNumber}</p>` +
	...
	sendSms: async(mobileNo, amount, accNo, refNo) => {
		...
		let maskAccNo = "";
		for (let x = 0; x < accNo.length; x++) {
			if (x <=3 ){
				maskAccNo += accNo.charAt(x);
			}if(x>3 && x < 11){
				maskAccNo += "#";
			} if (x > 11) {
				maskAccNo += accNo.charAt(x);
		...
		let MSent = `Sent ${amount} from UNIFIED to ${maskAccNo}. Reference: ${refNo}`;

...
class SmartpaymayaLog extends Controller {
	...
	findOne = async (args) => {
		...
		if (args.where.process == "resendotp") {
			...
			if ([1, 6].includes(user.userlevel)) {
				...
				balanceToPay = (parseFloat(paymayalog.amount) + parseFloat(paymayalog.charge)) - parseFloat(unilevelData.income);
			}
			...
			await DataManager._models.User.update({ ecashWallet: balanceAfter }, { where: { regcode }, transaction });
			...
			if (paymayalog.status == "POSTED" || (paymayalog.status == "OTP" && verifyResponse.status == 1)) {
				...
				if (executeRemittance.responseCode != "0000") {
					transaction.rollback();
					const remarks = "Error on executing remittance request";
					...
					try {
						...
						log(generalReport);
						const genRep = await DataManager._models.GeneralReport.create({
					...
					const updateLog = await DataManager._models.SmartpaymayaLog.update({
	...
	create = async (args) => {
		try {
			...
			if (user.ecashWallet < totalAmount) {
			...
			try {
				if (saveLogs.status == "OTP") {
					...
					if ([1, 6].includes(user.userlevel)) {
						balanceToPay = parseFloat(totalAmount) + parseFloat(res.charge) - parseFloat(unilevelData.income);
					} else {
						balanceToPay = parseFloat(totalAmount) + parseFloat(res.charge) - parseFloat(unilevelData.income);
					}
					...
					if (executeRemittance.responseCode != "0000") {
						...
						*/
						...
						let remarks = "An Error occur. Please try to create new remittance again.";
